msgfile='/Users/a0s01hy/work/projects/database/shellscripting/msg/gocofirmation-completed.json'
...
kcat -b 'kafka-1301504412-1-1334708561.scus.kafka-v2-atlas-secured-stg.ms-df-messaging.stg-az-southcentralus-10.prod.us.walmart.net:9093,kafka-1301504412-2-1334708564.scus.kafka-v2-atlas-secured-stg.ms-df-messaging.stg-az-southcentralus-10.prod.us.walmart.net:9093,kafka-1301504412-3-1334708567.scus.kafka-v2-atlas-secured-stg.ms-df-messaging.stg-az-southcentralus-10.prod.us.walmart.net:9093' -H facilityNum=4008 -H facilityCountryCode=US  -H userId=akashshukla -H eventType=FULFILLMENT_CONFIRMATION -t hawkeye-dev-amr-hvfc-fulfillment-confirmation -F /etc/secrets/kafka-secure-dev.properties
secure_FO_MSG(){
   echo `cat $msgfile`
    kcat -b $PUBLISHER_BROKER -H facilityNum=4008 -H facilityCountryCode=US  -H userId=akashshukla -H eventType=FULFILLMENT_CONFIRMATION -t hawkeye-dev-amr-hvfc-fulfillment-confirmation -P $msgfile
    echo "message sent"
}

FO_MSG(){
   echo `cat $msgfile`
    kcat -b $PUBLISHER_BROKER -H facilityNum=4008 -H facilityCountryCode=US  -H userId=akashshukla -H eventType=FULFILLMENT_NOTIFICATION -t hawkeye-dev-amr-hvfc-fulfillment-confirmation -P $msgfile
    echo "message sent"
}

OP_TO_FES(){
   echo `cat $msgfile`
    kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US  -H userId=akashshukla -t hawkeye-dev-rdc-putaway-response -P $msgfile
    echo "message sent"
}
...
#  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US -t ATLAS_WMSOP_FMS_ORDERS_ACK_PROD -P $msgfile
#  echo "message sent"
}
...
FES_TO_OT_PICKING() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H dcNumber=4034 -H countryCode=US -H facilityCountryCode=US -t yms2-gate-out -P $msgfile
  echo "message sent"
}
...
FES_TO_ACL() {
  echo $msgfile
  echo $$PUBLISHER_BROKER
  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US -H eventId=testingeventId -t hawkeye-dev-egm-label-verification-response -P $msgfile
  echo "message sent"
}

YMS_TO_OT_LOADING() {
  echo `cat $msgfile`
  kcat -b $PUBLISHER_BROKER  -H facilityNum=4034 -H facilityCountryCode=US  -H userId=akashshukla -t watcher-loading-package-event-p3 -P $msgfile
  echo "message sent"
}

YMS_TO_OT_GATEOUT() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US -H WMT_EventType=GATE_OUT_TEST_6 -H dcNumber=4034 -H countryCode=US -t yms2-gate-out -P $msgfile
}

FES_TO_WATCHER_SORTER() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -H site_name= -H WMT_CorrelationId=a660bff7-3788-404b-ab45-1e8344461248 -H dc_number=4034 -H country=US -H event_id=eca57b43-059b-485d-92b1-3b1b8e87bbe0 -H schema_id= -H event_name=shipping_label_generation -H producer_id= -H facilityCountryCode=US -H bu_type=MCC -H facilityNum=4034 -H timestamp=1629818382536 -t ATLAS_FC_OF_YMS_SHIPLABELGENERATION_STG -P $msgfile
  echo "message sent"
}

FES_TO_INVENTORY_MSG() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US -H WMT_CorrelationId=testak22 -H WMT_EventType=PickTransferredToConsolidation -H WMT_IdempotencyKey=test-b18-753f-4ae7-91bb-4b4a6643077fPW001B011 -t ATLAS_FC_OF_INVENTORYUPDATE_QA -P $msgfile
  echo "message sent"
}

kafkaProducer-dev() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -H facilityNum=4034 -H facilityCountryCode=US -H WMT_EventType=InvAdjusted -H WMT_IdempotencyKey=pickid-1-InvAdjusted -t ATLAS_FC_OF_INVENTORYUPDATE_QA -P $msgfile
  echo "message sent"
}

kafkaConsumer() {
  echo $msgfile
  kcat -b $PUBLISHER_BROKER -t CSII_NGOF_FULFILLMENTEVENT_DEV -C
}
...
prod_OF_publish() {
  echo `cat $msgfile`
      kcat -P -b $PUBLISHER_BROKER -H eventType=FULFILLMENT_UPDATE -H WMT_UserId=h0s07ef -H WMT_EventTs=2022-04-14 00:05:37.343 -H WMT_CorrelationId=test3e2e-500a-470c-b61e-54bcd264ba43 -H facilityNum=4034 -H facilityCountryCode=US -t GLS_ATLAS_WMSOF_FULFILLMENT_PROD $msgfile
      echo "message sent"
}
