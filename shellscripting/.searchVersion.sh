...
show_search_help() {
    ...
    echo -e "  search mode: search the input in files in key value pair or search string"
    echo -e "  sledge mode: sledge the version or details of application based on kitt info"
    echo -e "  show mode: show the info in tabular format like namespace, artifact, environment and cluster_id"
    echo ""
    echo -e "${STD_YEL}Usage:${RST}"
    echo -e "   ${STD_CYN}search${RST} <files> [search_input | env] [{-kv} | {-s|-details | -y} | {-show} ]"
    echo -e "   <files> | ${STD_CYN}search${RST} [search_input | env] [{-kv} | {-s|-details | -y} | {-show} ]"
    echo ""
    echo -e "${STD_YEL}flags:${RST}"
    echo -e "  -kv | --key-value       [SEARCH]                   Explicitly enable key-value search mode"
    echo -e "  -s | --sledge           [SLEDGE]                   Run sledge commands for version information"
    echo -e "  -details                [SLEDGE]                   Extract detailed information (version, dashboard, logging URLs)"
    echo -e "  -y | --yes.             [SLEDGE]                   Auto-execute sledge commands without prompt"
